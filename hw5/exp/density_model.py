...
class RBF(Density_Model):
    ...
    def get_prob(self, states):
        ...
        if self.means is None:
            ...
            # 1. Compute deltas
            states_ = np.reshape(states, [b, 1, ob_dim])
            states_ = np.tile(states_, [1, B, 1])
            deltas = states_ - self.means
...
class Exemplar(Density_Model):
    ...
    def make_prior(self, z_size):
        ...
        """
        prior_mean = tf.zeros([z_size], dtype=tf.dtypes.float32)
        prior_logstd = tf.ones([z_size], dtype=tf.dtypes.float32)
    ...
    def update(self, state1, state2, target):
        ...
        ll, kl, elbo, _ = self.sess.run([self.log_likelihood, self.kl, self.elbo, self.update_op], feed_dict) 
        return ll, kl, elbo
