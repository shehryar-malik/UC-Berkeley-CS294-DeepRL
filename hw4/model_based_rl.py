...
class ModelBasedRL(object):
    ...
    def _train_policy(self, dataset):
        ...
        ### PROBLEM 1
        ### YOUR CODE HERE
        for i in range(self._training_epochs):                                                                            # (a)
            #logger.info("Epoch: %d" % i)
            for j, (states, actions, next_states, _, _) in enumerate(dataset.random_iterator(self._training_batch_size)): # (b,c)
                loss = self._policy.train_step(states, actions, next_states)
                losses.append(loss)                                                                                       # (d)
                #if j % 100:
                #    logger.info("Iteration: %d, Loss: %f" % (j, loss))
    ...
    def run_q1(self):
        ...
        ### PROBLEM 1
        ### YOUR CODE HERE
        self._train_policy(self._random_dataset)        # (a)
    ...
    def run_q3(self):
        ...
        for itr in range(self._num_onpolicy_iters + 1):
            ...
            onpolicy_dataset = self._gather_rollouts(self._policy, self._num_onpolicy_rollouts)
