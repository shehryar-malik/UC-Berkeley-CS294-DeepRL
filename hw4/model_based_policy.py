...
class ModelBasedPolicy(object):
    ...
    def _dynamics_func(self, state, action, reuse):
        ...
        # (b)
        concat = tf.concat([state_norm, action_norm], axis=-1)

        # (c)
        norm_diff = utils.build_mlp(concat,
                                    self._state_dim,
                                    scope="norm_diff",
        ...
        # (d)
        delta_state_mean, delta_state_std = self._init_dataset.delta_state_mean, self._init_dataset.delta_state_std
        diff = utils.unnormalize(norm_diff, delta_state_mean, delta_state_std)

        next_state_pred = state + diff
    ...
    def _setup_training(self, state_ph, next_state_ph, next_state_pred):
        ...
        norm_actual_diff = utils.normalize(actual_diff, state_mean, state_std)
        norm_pred_diff = utils.normalize(pred_diff, state_mean, state_std)

        # (c)
        loss = tf.reduce_mean(tf.reduce_sum((norm_actual_diff - norm_pred_diff)**2, axis=-1))
    ...
    def _setup_action_selection(self, state_ph):
        ...
        action_seq = tf.random_uniform([self._num_random_action_selection, self._horizon, self._action_dim],
        ...
        best_seq_arg = tf.arg_min(costs, -1)
        best_action = action_seq[best_seq_arg, 0, :]
