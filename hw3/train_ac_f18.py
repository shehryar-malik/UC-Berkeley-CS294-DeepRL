...
#import tensorflow_probability as tfp       # this is not being used anywhere
import gym
import logz
import os
import time
...
def build_mlp(input_placeholder, output_size, scope, n_layers, size, activation=tf.tanh, output_activation=None):
    ...
    """
    # YOUR HW2 CODE HERE
    h_out = input_placeholder
    with tf.variable_scope(scope):
        for i in range(n_layers):
            h_out = tf.layers.dense(h_out,
                                    size,
                                    activation=activation,
                                    name="h_{}".format(i))

        output_placeholder = tf.layers.dense(h_out,
                                             output_size,
...
class Agent(object):
    ...
    def sample_action(self, policy_parameters):
        ...
        if self.discrete:
            ...
            # YOUR_HW2 CODE_HERE
            # Use the Gumbel-Max trick (see: https://timvieira.github.io/blog/post/2014/07/31/gumbel-max-trick/)
            z = -tf.log(-tf.log(tf.random_uniform(tf.shape(sy_logits_na))))
            sy_sampled_ac = tf.argmax(tf.add(sy_logits_na, z), axis=1)

        else:
            sy_mean, sy_logstd = policy_parameters
            # YOUR_HW2 CODE_HERE
            samples = tf.random_normal(tf.shape(sy_mean))
            sy_sampled_ac = tf.add(sy_mean, tf.multiply(samples, tf.exp(sy_logstd)))
    ...
    def get_log_prob(self, policy_parameters, sy_ac_na):
        ...
        if self.discrete:
            sy_logits_na = policy_parameters
            # YOUR_HW2 CODE_HERE
            log_sum = tf.log(tf.reduce_sum(tf.exp(sy_logits_na), axis=1))
            mask = tf.one_hot(sy_ac_na, self.ac_dim, dtype=sy_logits_na.dtype)
            sy_logprob_n = tf.reduce_sum(tf.multiply(sy_logits_na, mask), axis=1) - log_sum
        else:
            sy_mean, sy_logstd = policy_parameters
            # YOUR_HW2 CODE_HERE
            # Either use tensorflow's multivariate distribution
            mvn = tf.contrib.distributions.MultivariateNormalDiag(loc=sy_mean, scale_diag=tf.exp(sy_logstd))
            sy_logprob_n = mvn.log_prob(value=sy_ac_na)
            # Or manually calculate the log probability. This however only works when ac_dim=1
            #normed = (sy_ac_na - sy_mean)/tf.exp(self.logstd)
            #sy_logprob_n = - 0.5*tf.reduce_sum(normed*normed, axis=1) - 0.5*self.ac_dim*tf.log(2*np.pi) 
        return sy_logprob_n
    ...
    def sample_trajectory(self, env, animate_this_episode):
        ...
        while True:
            if animate_this_episode:
                ...
                time.sleep(0.1)
            obs.append(ob)
            ac = self.sess.run(self.sy_sampled_ac, {self.sy_ob_no: np.array([ob])}) # YOUR HW2 CODE HERE
            ac = ac[0]
            acs.append(ac)
    ...
    def estimate_advantage(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        # YOUR CODE HERE
        V_t   = self.sess.run(self.critic_prediction, {self.sy_ob_no: ob_no})
        V_tp1 = self.sess.run(self.critic_prediction, {self.sy_ob_no: next_ob_no})
    ...
    def update_critic(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        for _ in range(self.num_target_updates):
            y = re_n + self.gamma * self.sess.run(self.critic_prediction, {self.sy_ob_no: next_ob_no}) * (1-terminal_n)

            for _ in range(self.num_grad_steps_per_target_update):
                self.sess.run(self.critic_update_op, {self.sy_ob_no: ob_no, self.sy_target_n: y})
